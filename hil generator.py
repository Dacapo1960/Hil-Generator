...
SYSTEM_PROMPT = """You are an Airbus A320/A321 MEL (Master Minimum Equipment List) expert.
Given a defect description, return ONLY a raw JSON object. No markdown fences, no explanation.

{
  "mel_ref":          "e.g. 28-22-01",
  "mel_category":     "CAT A|B|C|D",
  "ata_chapter":      "e.g. ATA 28 - Fuel",
  "duration_days":    10,
  "discrepancy":      "Formal 1-2 sentence MEL discrepancy statement",
  "flight_limitations": "MEL operational limitations, or Nil",
  "has_o_proc":       true,
  "o_procedures":     ["Step 1", "Step 2", "Step 3", "Step 4 (consequence of further failure + required crew action)"],
  "has_m_proc":       true,
  "m_procedures":     "Maintenance action text"
}

Rules:
- Be accurate for Airbus A320/A321 MMEL.
- O-procedures: 4-5 steps. Step 1: verify on ECAM/SD page. Step 2: crew brief.
  Last 1-2 steps describe the consequence of a subsequent related failure and the required crew action.
- M-procedures: concise maintenance action including test requirements.
...
with st.form("hil_form"):
    ...
    with col1:
        ...
        operator     = st.text_input("Operator",     value="ASL GmbH / Trainingsflug")
    with col2:
        ac_type   = st.text_input("Aircraft Type",
                                  value="Airbus A320-214 (CFM 56-5B4)")
